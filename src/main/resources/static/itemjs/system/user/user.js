        ...
        },
        {
            ...
            formatter: function(value, row, index) {
            	if(row.userType == "Y") {
                	...
                	actions.push('<a class="btn btn-success btn-sm ' + resetFlag + '"  href="#" title="重置" onclick="resetPwd(\'' + row.id + '\')"><i class="fa fa-key"></i></a>');
                	return actions.join('');
				} else {
					return "";
				}
...
function queryDeptTreeDaTa()
{
	...
	var setting = {
		...
		$.get(ctx + "sys/dept/treeData", function(data) {
